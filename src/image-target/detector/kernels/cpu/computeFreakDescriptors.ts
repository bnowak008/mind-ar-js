...
function computeFreakDescriptorImpl(extremaFreaks: ExtremaFreaksData, positionT: PositionData): Float32Array {
    const resultValues = new Float32Array(extremaFreaks.height* descriptorCount); 
    function getP(y: number, x: number): number {
        return positionT.values[y*positionT.width+x];
    }
    function getFreak(y: number, x: number): number {

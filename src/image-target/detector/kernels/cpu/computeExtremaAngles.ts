...
type ComputeExtremaAnglesArgs = {
  inputs: { histograms: TensorInfo };
  backend: MathBackendCPU;
};

export function computeExtremaAnglesImpl(histogram: HistogramData): Float32Array {
...
export const computeExtremaAngles = (args: ComputeExtremaAnglesArgs): TensorInfo => {
    const { histograms } = args.inputs;
    const backend = args.backend;

    const histogramValues: HistogramData = {values: backend.data.get(histograms.dataId).values,width:histograms.shape[1],height:histograms.shape[0]};

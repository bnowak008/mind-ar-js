import {cumsum, type Cumsum} from '../utils/cumsum';
 ...
 */
const extract = (image: {data: Uint8Array, width: number, height: number, scale: number}) => {
  ...
  const imageDataSqr = new Array(imageData.length);
  for (let i = 0; i < imageData.length; i++) {
    imageDataSqr[i] = imageData[i] * imageData[i];
  }
  const imageData2D = Array.from({length: height}, (_, y) => 
    Array.from({length: width}, (_, x) => imageData[y * width + x])
  );
  const imageDataSqr2D = Array.from({length: height}, (_, y) => 
    Array.from({length: width}, (_, x) => imageDataSqr[y * width + x])
  );
  const imageDataCumsum = cumsum(imageData2D, width, height);
  const imageDataSqrCumsum = cumsum(imageDataSqr2D, width, height);
...
const _selectFeature = (options: {image: {data: Uint8Array, width: number, height: number, scale: number}, featureMap: Float32Array, templateSize: number, searchSize: number, occSize: number, maxSimThresh: number, minSimThresh: number, sdThresh: number, imageDataCumsum: Cumsum, imageDataSqrCumsum: Cumsum}) => {
...
const _templateVar = ({image, cx, cy, sdThresh, imageDataCumsum, imageDataSqrCumsum}: {image: {data: Uint8Array, width: number, height: number, scale: number}, cx: number, cy: number, sdThresh: number, imageDataCumsum: Cumsum, imageDataSqrCumsum: Cumsum}) => {
...
const _getSimilarity = (options: {image: {data: Uint8Array, width: number, height: number, scale: number}, cx: number, cy: number, vlen: number, tx: number, ty: number, imageDataCumsum: Cumsum, imageDataSqrCumsum: Cumsum}) => {

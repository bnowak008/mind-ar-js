...
class Tracker {
  ...
  debugMode: boolean;
  trackingKeyframeList: TrackingFrame[];
  featurePointsListT: tf.Tensor[];
  imagePixelsListT: tf.Tensor[];
  imagePropertiesListT: tf.Tensor[];
  kernelCaches: KernelCache;

  constructor(markerDimensions: [number, number][], trackingDataList: TrackingData[], projectionTransform: number[][], inputWidth: number, inputHeight: number, debugMode: boolean = false) {
  ...
  track(inputImageT: tf.Tensor, lastModelViewTransform: number[][], targetIndex: number) {
    let debugExtra = {};
    ...
    let matchingPoints = matchingPointsT.arraySync();
    let sim = simT.arraySync();
    ...
    if (typeof matchingPoints === 'number') {
    ...
    const extractNumber = (value: any): number | null => {
      ...
      if (Array.isArray(value) && value.length > 0) return extractNumber(value[0]);
      return null;
    };

    const extractCoordinates = (value: any): [number, number] => {
      if (typeof value === 'number') return [value, 0];
      if (Array.isArray(value) && value.length >= 2) {
        const x = extractNumber(value[0]);
        const y = extractNumber(value[1]);
    ...
    for (let i = 0; i < matchingPoints.length; i++) {
      const simValue = extractNumber(sim[i]);
      if (simValue !== null && simValue > AR2_SIM_THRESH && i < trackingFrame.points.length) {
        goodTrack.push(i);
        const coords = extractCoordinates(matchingPoints[i]);
        const point = computeScreenCoordiate(modelViewProjectionTransform, coords[0], coords[1], 0);
  ...
  _computeMatching(featurePointsT: tf.Tensor, imagePixelsT: tf.Tensor, imagePropertiesT: tf.Tensor, projectedImageT: tf.Tensor) {
    ...
    return tf.tidy(() => {
      ...
      if (!programs) {
        throw new Error('computeMatching programs not found');
      }
  ...
  _computeProjection(modelViewProjectionTransformT: tf.Tensor, inputImageT: tf.Tensor, targetIndex: number) {
    ...
    return tf.tidy(() => {
      if (!this?.kernelCaches?.computeProjection?.[kernelKey]) {
        throw new Error('computeProjection program not found');
      }
  ...
  _prebuild(trackingFrame: TrackingFrame, maxCount: number) {
    return tf.tidy(() => {

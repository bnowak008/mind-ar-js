...
const computeHoughMatches = (options: {keywidth: number, keyheight: number, querywidth: number, queryheight: number, matches: {querypoint: {scale: number, angle: number, x: number, y: number}, keypoint: {scale: number, angle: number, x: number, y: number}}[]}) => {
  ...
  const projectedDims = new Array<number>();
  ...
  Object.keys(votes).forEach((index) => {
    ...
    if (votes[numIndex] > maxVotes) {
      maxVotes = votes[numIndex];
      maxVoteIndex = numIndex;
    }
  });
...
const _mapCorrespondence = ({querypoint, keypoint, keycenterX, keycenterY, scaleOneOverLogK}: {querypoint: {scale: number, angle: number, x: number, y: number}, keypoint: {scale: number, angle: number, x: number, y: number}, keycenterX: number, keycenterY: number, scaleOneOverLogK: number}) => {
  // map angle to (-pi, pi]

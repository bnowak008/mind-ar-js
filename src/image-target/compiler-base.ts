...
import { buildImageList, buildTrackingImageList, ImageList } from './image-list.js';
...
class CompilerBase {
  ...
  // input html Images
  compileImageTargets(images: HTMLImageElement[], progressCallback: (progress: number) => void) {
    return new Promise(async (resolve, reject) => {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const targetImage = { data: new Float32Array(greyImageData), height: img.height, width: img.width };
      ...
      for (let i = 0; i < this.data.length; i++) {
        const trackingImageList = buildTrackingImageList(this.data[i].targetImage);
  ...
  importData(buffer: Uint8Array) {
    const content = msgpack.decode(new Uint8Array(buffer)) as {v: number, dataList: any[]};
    ...
    for (let i = 0; i < dataList.length; i++) {
      this.data.push({
        ...
        imageList: buildImageList(dataList[i].targetImage)
      });
    }
    return this.data;
  }
  ...
  compileTrack({progressCallback, targetImages, basePercent}: {progressCallback: (progress: number) => void, targetImages: ImageData[], basePercent: number}): Promise<any> {
    // sub-class implements
...
const _extractMatchingFeatures = async (imageList: ImageList, doneCallback: (index: number) => void) => {
  const keyframes: any[] = [];

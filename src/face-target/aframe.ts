...
interface ComponentWithMatrix extends AFRAME.Component {
  updateVisibility(visible: boolean): void;
  updateMatrix(matrix: number[]): void;
}

interface FaceMeshComponent extends ComponentWithMatrix {
...
AFRAME.registerSystem('mindar-face-system', {
  ...
  setup: function({showStats, uiLoading, uiScanning, uiError, filterMinCF, filterBeta, disableFaceMirror}: {showStats: boolean, uiLoading: string, uiScanning: string, uiError: string, filterMinCF: number, filterBeta: number, disableFaceMirror: boolean}) {
  ...
  registerFaceMesh: function(el: FaceMeshComponent) {
    this.faceMeshEntities.push({el});
  },

  registerAnchor: function(el: ComponentWithMatrix, anchorIndex: number) {
  ...
  _processVideo: function() {
    this.controller.onUpdate = ({hasFace, estimateResult}: {hasFace: boolean, estimateResult: {faceMatrix: number[]}}) => {
...
AFRAME.registerComponent('mindar-face-occluder', {
  init: function() {
    ...
    this.el.addEventListener('model-loaded', () => {
      this.el.getObject3D('mesh').traverse((o: THREE.Object3D) => {
...
AFRAME.registerComponent('mindar-face-default-face-occluder', {
  ...
  addFaceMesh(faceGeometry: THREE.BufferGeometry) {

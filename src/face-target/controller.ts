...
class Controller {
  ...
  constructor({onUpdate=null, filterMinCF=null, filterBeta=null}: {onUpdate?: ((data: any) => void) | null, filterMinCF: number | null, filterBeta: number | null}) {
  ...
  onInputResized(input: HTMLVideoElement | HTMLImageElement) {
    this.estimator = new Estimator(input);
  }

  getCameraParams() {
    if (!this.estimator) {
      return { fov: 0, aspect: 1, near: 0.1, far: 1000 };
    }
    return {
  ...
  async dummyRun(input: HTMLVideoElement | HTMLImageElement) {
    await this.faceMeshHelper.detect(input);
  }


  processVideo(input: HTMLVideoElement | HTMLImageElement) {
	...
	const landmarks = results.faceLandmarks[0].map((l: {x: number, y: number, z: number}) => {
  ...
  createThreeFaceGeometry(THREE: any) {
    const faceGeometry = _createThreeFaceGeometry(THREE);
    this.customFaceGeometries.push(faceGeometry as FaceGeometry);
    return faceGeometry;
  }
